...
Proof. 
  ...
  rewrite <- (f_equal ((fun n m (e: syntax.expr _ _ n m) => 
    match e in syntax.expr _ _ n m return syntax.expr _ _ n m
      with e_pls _ _ x _ => x | x => x end) _ _) E). 
  rewrite <- (f_equal ((fun n m (e: syntax.expr _ _ n m) => 
    match e in syntax.expr _ _ n m return syntax.expr _ _ n m
      with e_pls _ _ _ y => y | x => x end) _ _) E). 
  eauto. 
Qed.
...
Proof. 
  ...
  rewrite <-(f_equal ((fun n m (e: syntax.expr _ _ n m) => 
    match e in syntax.expr _ _ n m return syntax.expr _ _ n m
      with e_itr _ y => y | x => x end) _ _) Hv).
...
Proof. 
  ...
  rewrite <-(f_equal ((fun n m (e: syntax.expr _ _ n m) => 
    match e in syntax.expr _ _ n m return syntax.expr _ _ n m
      with e_str _ y => y | x => x end) _ _) Hv).
  eexists. split. eassumption. now rewrite cmp_eq_rect_eq. 
  intro Hz. apply (G _ _ _ Hz eq_refl _ eq_refl). 
Qed.
